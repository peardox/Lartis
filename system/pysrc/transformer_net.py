...
def checkabort():
    if ioopts.StyleAbortFlag:
        print('Abort signaled')
        ioopts.StyleAbortFlag = False
        pstyle.StyleAbort()
        exit()
    return False
...
class TransformerNet(torch.nn.Module):
    ...
    def forward(self, X):
        if self.showTime and have_delphi_style and  have_delphi_io:
            ioopts.StyleJsonLog = json.dumps(TJsonLog(event = 'styleTime', subevent = 'Controller', time = time.time() - self.timeStamp))
...
class ConvLayer(torch.nn.Module):
    ...
    def forward(self, x):
        if self.showTime and have_delphi_style and  have_delphi_io:
            ioopts.StyleJsonLog = json.dumps(TJsonLog(event = 'styleTime', subevent = 'ConvLayer', time = time.time() - self.timeStamp))
            pstyle.StyleProgress()
            if checkabort():
                return
...
class ResidualBlock(torch.nn.Module):
    ...
    def forward(self, x):
        if self.showTime and have_delphi_style and  have_delphi_io:
            ioopts.StyleJsonLog = json.dumps(TJsonLog(event = 'styleTime', subevent = 'ResidualBlock', time = time.time() - self.timeStamp))
            pstyle.StyleProgress()
            if checkabort():
                return
        residual = x
...
class UpsampleConvLayer(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.upsample:
            if self.showTime and have_delphi_style and  have_delphi_io:
                ioopts.StyleJsonLog = json.dumps(TJsonLog(event = 'styleTime', subevent = 'UpSample', time = time.time() - self.timeStamp))
                pstyle.StyleProgress()
                if checkabort():
                    return
