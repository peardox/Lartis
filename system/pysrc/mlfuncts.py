            ...
            for batch_id, (x, _) in enumerate(train_loader):
                ...
                if train_interval > reporting_interval:
                    ...
                    if(args.log_event_api):
                        if have_delphi_train and  have_delphi_io:
                            ioopts.TrainJsonLog = json.dumps(TJsonLog(
                        ...
                        if (1): # sbdbg (not have_delphi_train) and (not args.calibrating): 
...
#        print("FMX available")
#    else:
#        print("FMX unavailable")
    
    if args.content_image != "":
        content_image = utils.load_image(args.content_image, args.content_scale)
    else:
        content_image = utils.load_raw(args.content_image_raw, args.content_scale)
    ...
    if args.model.endswith(".onnx"):
        ...
        with torch.no_grad():
            ...
            if have_delphi_style and have_delphi_io:
                ioopts.StyleJsonLog = json.dumps(TJsonLog(event = 'styleTime', subevent = 'StartStyle', time = 0))
                pstyle.StyleProgress()
            ...
            if args.export_onnx:
                ...
                if have_delphi_style and have_delphi_io:
                    ioopts.StyleJsonLog = json.dumps(TJsonLog(event = 'styleTime', subevent = 'EndStyle', time = time.time() - style_time))
                    pstyle.StyleProgress()
            else:
    ...
    if have_delphi_style and have_delphi_io:
        ioopts.StyleFilename = args.output_image
        ioopts.ProcessRuntime = time.time() - style_time
        pstyle.StyleFinished()
